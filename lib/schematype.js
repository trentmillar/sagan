...
/**
 * Declares the index extend for this schematype.
 *
 * ####Example:
 *
 ...
 *
 * Setters are also passed a second argument, the schematype on which the setter was defined. This allows for tailored behavior based on extend passed in the schema.
 *
 *     function inspector (val, schematype) {
 *       if (schematype.extend.required) {
 ...
 *     });
 *
 * Getters are also passed a second argument, the schematype on which the getter was defined. This allows for tailored behavior based on extend passed in the schema.
 *
 *     function inspector (val, schematype) {
 *       if (schematype.extend.required) {
 *         return schematype.path + ' is required';
...
SchemaType.prototype.validate = function(obj, message, type) {
  if (typeof obj === 'function' || obj && utils.getFunctionName(obj.constructor) === 'RegExp') {
    ...
    if (message instanceof Object && !type) {
      ...
      if (!message) {
        message = 'TODO';//MongooseError.messages.general.default;
      }
      if (!type) {
        type = 'user defined';
      }
 ...
 *
 * @param {Boolean|Function|Object} required enable/disable the validator, or function that returns required boolean, or extend object
 * @param {Boolean|Function} [extend.isRequired] enable/disable the validator, or function that returns required boolean
 * @param {Function} [extend.ErrorConstructor] custom error constructor. The constructor receives 1 parameter, an object containing the validator properties.
...
SchemaType.prototype.required = function(required, message) {
  ...
  var msg = message || 'TODO';//MongooseError.messages.general.required;
...
SchemaType.prototype.doValidate = function(value, fn, scope) {
  ...
  var validate = function(ok, validatorProperties) {
    ...
    if (ok === undefined || ok) {
      ...
      var ErrorConstructor = validatorProperties.ErrorConstructor || null;//ValidatorError;
