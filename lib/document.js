...
/*const internalToObjectOptions = require('./extend').internalToObjectOptions;
...
function Document(obj, fields, skipId, options) {
  ...
  if (obj != null && typeof obj !== 'object') {
    throw new /*ObjectParameterError*/Error(obj + ' obj' + 'Document');
  }
...
['on', 'once', 'emit', 'listeners', 'removeListener', 'setMaxListeners',
  'removeAllListeners', 'addListener'].forEach(function(event) {
  var emitter = function(emitterFn) {
 ...
 *
 * @api public
 * @see Schema extend /docs/guide.html#extend
 * @property id
 * @memberOf Document
 * @instance
 */
 ...
 * @param {Object} [options] optionally specify extend that modify the behavior of the set
 * @method $set
 * @name $set
 * @memberOf Document
 * @instance
...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  try {
    ...
    var setterContext = constructing && this.$__.$options.priorDoc ?
      ...
      this;
    val = schema.applySetters(val, setterContext, false, priorVal); // sets the value on the instance
    ...
    this.invalidate(path,
      new /*MongooseError.CastError*/Error(/*schema.instance, val, path, e*/));
    shouldSet = false;
  }
 ...
 * @param {Any} val the value to set
 * @param {Schema|String|Number|Buffer|*} [type] optionally specify a type for "on-the-fly" attributes
 * @param {Object} [extend] optionally specify extend that modify the behavior of the set
 * @api public
 * @method set
 * @memberOf Document
 * @instance
 ...
 *     });
 *
 * @param {Object} optional extend internal extend
 ...
 *     });
 *
 * @param {Object} [extend] extend optional extend
 * @param {Object} [extend.safe] overrides [schema's safe option](http://mongoosejs.com//docs/guide.html#safe)
 * @param {Boolean} [extend.validateBeforeSave] set to false to save without validating.
...
Document.prototype.$__setSchema = function(schema) {
  // TODO, this is unneeded with cosmos
/*
...
/**
 * Internal helper for toObject() and toJSON() that doesn't manipulate extend
 *
 * @api private
 * @method $toObject
 * @memberOf Document
...
Document.prototype.$toObject = function(options, json) {
  ...
  // merge base default extend with Schema's set default extend if available.
  // `clone` is necessary here because `utils.extend` directly modifies the second input.
  if (json && this.schema.options.toJSON) {
    defaultOptions = utils.extend(defaultOptions, clone(this.schema.options.toJSON));
  } else if (this.schema.options.toObject) {
    defaultOptions = utils.extend(defaultOptions, clone(this.schema.options.toObject));
  }

  // If extend do not exist or is not an object, set it to empty object
  ...
  // The original extend that will be passed to `clone()`. Important because
  // `clone()` will recursively call `$toObject()` on embedded docs, so we
  // need the original extend the user passed in, plus `_isNested` and
  ...
  // merge default extend with input extend.
  ...
  // check and see if the parent has a transform (extend.transform) and if the
  // child schema has a transform (this.schema.extend.toObject) In this case,
  // we need to adjust extend.transform to be the child schema's transform and
 ...
 *
 * To apply these extend to every document of your schema by default, set your [schemas](#schema_Schema) `toObject` option to the same argument.
 *
 ...
 *
 *     function (doc, ret, extend) {}
 *
 * - `doc` The mongoose document which is being converted
 * - `ret` The plain object representation which has been converted
 * - `extend` The extend in use (either schema extend or the extend passed inline)
 *
 * ####Example
 *
 *     // specify the transform schema option
 *     if (!schema.extend.toObject) schema.extend.toObject = {};
 *     schema.extend.toObject.transform = function (doc, ret, extend) {
 ...
 *
 * Transformations may also be applied inline, overridding any transform set in the extend:
 *
 *     function xform (doc, ret, extend) {
 ...
 *
 *     if (!schema.extend.toObject) schema.extend.toObject = {};
 *     schema.extend.toObject.hide = '_id';
 *     schema.extend.toObject.transform = function (doc, ret, extend) {
 *       if (extend.hide) {
 *         extend.hide.split(' ').forEach(function (prop) {
 *           delete ret[prop];
 *         });
 *       }
 *       return ret;
 ...
 *
 * Transforms, like all of these extend, are also available for `toJSON`.
 *
 * See [schema extend](/docs/guide.html#toObject) for some more details.
 *
 * _During save, no custom extend are applied to the document before being sent to the database._
 *
 * @param {Object} [options]
 ...
 *
 * This method accepts the same extend as [Document#toObject](#document_Document-toObject). To apply the extend to every document of your schema by default, set your [schemas](#schema_Schema) `toJSON` option to the same argument.
 *
 *     schema.set('toJSON', { virtuals: true })
 *
 * See [schema extend](/docs/guide.html#toJSON) for details.
 *
 * @param {Object} options
 * @return {Object}
 ...
 *     doc.populate(extend);               // not executed
 *     doc.populate(path, callback)         // executed
 *     doc.populate(extend, callback);     // executed
 *     doc.populate(callback);              // executed
 *     doc.populate(extend).execPopulate() // executed, returns promise
 *
 *
 * ####NOTE:
 *
 ...
 * call `execPopulate()`.
 * Passing the same path a second time will overwrite the previous path extend.
 * See [Model.populate()](#model_Model.populate) for explaination of extend.
 *
 * @see Model.populate #model_Model.populate
 * @see Document.execPopulate #document_Document-execPopulate
 * @param {String|Object} [path] The path to populate or an extend object
