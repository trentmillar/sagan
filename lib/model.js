 ...
 * @event `index-single-start`: Emitted when an individual index starts within `Model#ensureIndexes`. The fields and extend being used to build the index are also passed with the event.
 * @event `index-single-done`: Emitted when an individual index finishes within `Model#ensureIndexes`. If an error occurred it is passed with the event. The fields, extend, and index name are also passed.
 * @api public
 */

function Model(doc, fields, skipId) {
  if (fields instanceof Schema) {
    throw new Error/*TypeError*/('2nd argument to `Model` must be a POJO or string, ' +
...
Model.prototype.$__handleSave = function(options, callback) {
  ...
  if (this.isNew) {
    ...
    this./*collection.insertOne*/db.cosmosDbClient.upsertDocument(obj, safe, function(err, ret) {
      if (err) {
        _this.isNew = true;
 ...
 *     });
 *
 * @param {Object} [options] extend optional extend
...
Model.prototype.save = function(options, fn) {
  ...
  if (this.$__.saving) {
    ...
    this.$__.saving = new /*ParallelSaveError*/Error(this).stack;
  }
...
function checkDivergentArray(doc, path, array) {
  ...
  // If the array was populated using extend that prevented all
 ...
 *
 * @param {Object} [options] internal extend
 ...
 *
 *     // passing extend and executing immediately
 ...
 *
 *     // passing extend (in this case return the raw js objects, not mongoose documents by passing `lean`
 ...
 *
 *     // specify extend, in this case lean
 ...
 *
 * Finds a matching document, updates it according to the `update` arg, passing any `extend`, and returns the found document (if any) to the callback. The query executes immediately if `callback` is passed else a Query object is returned.
 *
 * ####Options:
 *
 ...
 * ####Examples:
 *
 *     A.findOneAndUpdate(conditions, update, extend, callback) // executes
 *     A.findOneAndUpdate(conditions, update, extend)  // returns Query
 ...
 *     Model.findOneAndUpdate(query, { name: 'jason bourne' }, extend, callback)
 *
 *     // is sent as
 *     Model.findOneAndUpdate(query, { $set: { name: 'jason bourne' }}, extend, callback)
 *
 ...
 * enable these by setting the `runValidators` extend,
 * respectively.
 *
  ...
  if (typeof options === 'function') {
    ...
    if (typeof conditions === 'function') {
      var msg = 'Model.findOneAndUpdate(): First argument must not be a function.\n\n'
				+ '  ' + this.modelName + '.findOneAndUpdate(conditions, update, extend, callback)\n'
				+ '  ' + this.modelName + '.findOneAndUpdate(conditions, update, extend)\n'
 ...
 * passing any `extend`, and returns the found document (if any) to the
 ...
 * ####Examples:
 *
 *     A.findByIdAndUpdate(id, update, extend, callback) // executes
 *     A.findByIdAndUpdate(id, update, extend)  // returns Query
 ...
 * ####Example:
 *
 *     Model.findByIdAndUpdate(id, { name: 'jason bourne' }, extend, callback)
 *
 *     // is sent as
 *     Model.findByIdAndUpdate(id, { $set: { name: 'jason bourne' }}, extend, callback)
 *
 * This helps prevent accidentally overwriting your document with `{ name: 'jason bourne' }`.
 *
 * ####Note:
 ...
 * ####Examples:
 *
 *     A.findOneAndDelete(conditions, extend, callback) // executes
 *     A.findOneAndDelete(conditions, extend)  // return Query
 ...
 * ####Examples:
 *
 *     A.findOneAndRemove(conditions, extend, callback) // executes
 *     A.findOneAndRemove(conditions, extend)  // return Query
 ...
 * ####Examples:
 *
 *     A.findByIdAndRemove(id, extend, callback) // executes
 *     A.findByIdAndRemove(id, extend)  // return Query
...
Model.upsertDocument = function upsertDocument() {
  let args = arguments;
};
 ...
 * @param {Object} [options] see the [mongodb driver extend](http://mongodb.github.io/node-mongodb-native/3.0/api/Collection.html#watch)
 ...
 *
 * @param {Object} [options] see the [mongodb driver extend](http://mongodb.github.io/node-mongodb-native/3.0/api/MongoClient.html#startSession)
 ...
 * @param {Object} [options] see the [mongodb driver extend](http://mongodb.github.io/node-mongodb-native/2.2/api/Collection.html#insertMany)
 ...
 *
 *     var query = { name: 'borne' };
 *     Model.update(query, { name: 'jason bourne' }, extend, callback)
 *
 *     // is sent as
 *     Model.update(query, { $set: { name: 'jason bourne' }}, extend, callback)
 ...
 *
 * `o` is an object specifying all mapReduce extend as well as the map and reduce functions. All extend are delegated to the driver implementation. See [node-mongodb-native mapReduce() documentation](http://mongodb.github.io/node-mongodb-native/api-generated/collection.html#mapreduce) for more detail about extend.
 *
 * This function does not trigger any middleware.
 *
 * ####Example:
 ...
 *
 * If `extend.out` is set to `replace`, `merge`, or `reduce`, a Model instance is returned that can be used for further querying. Queries run against this model are all executed with the `lean` option; meaning only the js object is returned and no Mongoose magic is applied (getters, setters, etc).
 *
 * ####Example:
 *
 *     var o = {};
 ...
 *
 * @param {Object} o an object specifying map-reduce extend
 ...
 * ####Example:
 *
 *     var extend = { near: [10, 10], maxDistance: 5 };
 *     Locations.geoSearch({ type : "house" }, extend, function(err, res) {
 *       console.log(res);
 *     });
 *
 * ####Options:
...
Model.geoSearch = function(conditions, options, callback) {
  ...
  return utils.promiseOrCallback(callback, cb => {
    ...
    // send the conditions in the extend object
 ...
 * - extend: optional query extend like sort, limit, etc
 *
 * ####Examples:
 *
 ...
 *         , { path: 'notes', extend: { limit: 10 }, model: 'override' }
 *       ]
 *
 ...
 * @param {Object} extend A hash of key/val (path, extend) used for population.
 * @param {Boolean} [extend.retainNullValues=false] by default, mongoose removes null and undefined values from populated arrays. Use this option to make `populate()` retain `null` and `undefined` array entries.
...
function _populate(model, docs, paths, cache, callback) {
  ...
  // each path has its own query extend and must be executed separately
  var i = pending;
  var path;
  while (i--) {
    path = paths[i];
...
function assignVals(o) {
  // Glob all extend together because `populateOptions` is confusing
  ...
  for (i = 0; i < len; i++) {
    ...
    if (virtual && (!localField || !foreignField)) {
      throw new Error('If you are populating a virtual, you must set the ' +
				'localField and foreignField extend');
    }
  ...
  function _getModelNames(doc, schema) {
    ...
    if (refPath) {
      ...
      if (!modelNameFromQuery) {
        ...
        modelNames = [modelNameFromQuery]; // query extend
      }
    }
...
Model.compile = function compile(name, schema, collectionName, client, base) {
  ...
  model.db = model.prototype.db = client;
  ...
  if (client.base.get('bufferCommands') != null) {
    bufferCommands = client.base.get('bufferCommands');
  }
  // Connection-specific overrides the global option
  if (client.config.bufferCommands != null) {
    bufferCommands = client.config.bufferCommands;
  }
  // And schema extend override global and connection
  ...
  var collectionOptions = {
    ...
    partition: schema.options.partition
  };

  model.prototype.collection = client.collection(
    collectionName
    , collectionOptions
  );
